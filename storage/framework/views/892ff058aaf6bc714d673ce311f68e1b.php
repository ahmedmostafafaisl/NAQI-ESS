<aside class="w-64 shrink-0 bg-slate-900 text-slate-300 flex flex-col">
    ...
    <nav class="flex-1 px-3 py-4 space-y-6 text-sm overflow-y-auto">
        ...
        <?php if(auth()->user()->can('users.view') || auth()->user()->can('roles.manage') || auth()->user()->can('permissions.manage') || auth()->user()->can('settings.view')): ?>
            <div>
                <p class="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                    <?php echo e(__('admin.nav.administration')); ?>
                ...
                <div class="space-y-1">
                    ...
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('settings.view')): ?>
                        <a href="<?php echo e(route('admin.settings.index')); ?>"
                            class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition <?php echo e(request()->routeIs('admin.settings.*') ? 'bg-indigo-600 text-white shadow-sm' : 'hover:bg-slate-800/70 hover:text-white'); ?>">
                            <span class="text-base">⚙️</span>
                            <span><?php echo e(__('admin.nav.settings')); ?></span>
                        </a>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('dynamics.sync')): ?>
            <div>
                <p class="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                    <?php echo e(__('admin.nav.integrations')); ?>

                </p>
                <a href="<?php echo e(route('admin.dynamics.index')); ?>"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition <?php echo e(request()->routeIs('admin.dynamics.*') ? 'bg-indigo-600 text-white shadow-sm' : 'hover:bg-slate-800/70 hover:text-white'); ?>">
                    <span class="text-base">🔗</span>
                    <span><?php echo e(__('admin.nav.dynamics')); ?></span>
                </a>
            </div>
        <?php endif; ?>
        ...
        <div>
            <p class="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                <?php echo e(__('admin.nav.engagement')); ?>
...
<?php /**PATH /home/faisal/new dev/DEV/Naqi/naqi-ess-prod/resources/views/components/sidebar.blade.php ENDPATH**/ ?>