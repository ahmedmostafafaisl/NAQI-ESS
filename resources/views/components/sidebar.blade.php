<aside class="w-64 shrink-0 bg-slate-900 text-slate-300 flex flex-col">
    ...
    <nav class="flex-1 px-3 py-4 space-y-6 text-sm overflow-y-auto">
        <div>
            <p class="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">{{ __('admin.nav.main') }}</p>
        ...
        @if(auth()->user()->can('users.view') || auth()->user()->can('roles.manage') || auth()->user()->can('permissions.manage'))
        <div>
            <p class="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">{{ __('admin.nav.administration') }}</p>
            <div class="space-y-1">
                @can('users.view')
        ...
        <div>
            <p class="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-500">{{ __('admin.nav.engagement') }}</p>
